...
public class TaskGenerateFLogisticoEstocastico extends TaskF {

	public TaskGenerateFLogisticoEstocastico(Config c, Opcao o) {
		super(c, o);
	}
	...
	public double calculaFijk(int i, int j, int k, double qStar) throws ConvergenceException {
		...
		if ((j == o.getMaxStepE() - 1) && (i == o.getMaxStepP() - 1)) {
			...
			double pjtermo1 = Math.pow(c.getSigmaE(), 2) * Math.pow(j, 2);
			double pjtermo2 = (((c.getVelRev() * (c.getEbarra() - (j * o.getDeltaE()))) * j * o.getDeltaE()) - qStar) / o.getDeltaE();
			double pujfinal = (pjtermo1 + pjtermo2) / 2;
			double pdjfinal = (pjtermo1 - pjtermo2) / 2;
			...
			double pmtermo2 = Math.pow(c.getSigmaE(), 2) * Math.pow(j, 2);
			double pmtermo3 = 1 / o.getDeltaT();
			double pmfinal = pmtermo3 - pmtermo1 - pmtermo2;
			double pitermo1 = Math.pow(c.getSigma(), 2) * Math.pow(i, 2);
			double pitermo2 = (c.getR() - c.getConvYield()) * i;
			double puifinal = (pitermo1 + pitermo2) / 2;
			double pdifinal = (pitermo1 - pitermo2) / 2;
	...
	public String taskName() {
		return "Calculating F Logístico Estocastico...";
	}
}
